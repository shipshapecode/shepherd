...
const PricingPage = () => {
  ...
  return (
    <>
      ...
      <section
  className="relative flex flex-col items-center w-full 2xl:max-w-7xl mx-auto mb-20">
  <div className="w-full mx-auto border-b-2 border-black">
    <div
      className="relative p-8 lg:p-20 2xl:px-0 items-center text-center justify-center max-w-3xl mx-auto">
      <h1 className="text-3xl lg:text-5xl tracking-tight font-heading text-black">
        Pricing options and packages
      </h1>
      <p className="max-w-lg mt-4 xl:text-xl text-lg text-black mx-auto">
        Come grow with us! Choose the plan that best suits your needs.
      </p>
      <div className="max-w-xs mx-auto mt-8">
        <div
          className="overflow-hidden inline-flex bg-white gap-1 text-black z-0 text-lg font-semibold border-2 border-black p-1 w-full shadow">
          <button
            className={cn("text-black w-full block px-8 py-2 h-14 transition", annual == false && 'bg-pink-200')}
            onClick={() => setAnnual(false)}
            type="button"
            >Monthly
          </button>
          <button
            className={cn("text-black w-full block px-8 py-2 h-14 transition", annual == true ? 'bg-pink-200 ' : '')}
    ...
    <div
      className="grid grid-cols-1 lg:grid-cols-2 gap-0.5 border-2 border-black bg-black">
      {
          ...
          <div
            ...
            <div className="bg-white border-2 border-black p-8 rounded-3xl shadow h-full">
              <div className="inline-flex items-center justify-between w-full font-semibold">
                <h2 className="text-3xl lg:text-6xl justify-between tracking-tight">
                  {plan.name}
                </h2>
              </div>
              <div className="text-3xl lg:text-6xl mt-8">
              {plan.annualPrice ? (
                <>$
                  <span>{annual ? plan.annualPrice : plan.monthlyPrice}</span>
              ...
              <p className="lg:text-xl text-sm tracking-wide mt-8">
                {plan.description}
              </p>
            </div>
            <ul className="mt-2 space-y-8 text-base leading-6 tracking-wide">
              {plan.features.map((feature) => (
                <li className="flex items-start">
                  ...
                  <p className="ml-6">
                    <span className="font-semibold">{feature.highlightText} ⏤</span>
                    {feature.text}
                  </p>
                </li>
              ))}
