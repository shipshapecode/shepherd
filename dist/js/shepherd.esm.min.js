...
function Aa(a){for(var b=A(a),c=za(a);c&&0<=["table","td","th"].indexOf(w(c))&&"static"===getComputedStyle(c).position;)c=za(c);if(c&&("html"===w(c)||"body"===w(c)&&"static"===getComputedStyle(c).position))return b;if(!c)a:{c=-1!==navigator.userAgent.toLowerCase().indexOf("firefox");for(a=ya(a);B(a)&&0>["html","body"].indexOf(w(a));){var d=getComputedStyle(a);if("none"!==d.transform||"none"!==d.perspective||"paint"===d.contain||-1!==["transform","perspective"].indexOf(d.willChange)||c&&"filter"===
